    ...
    return {
        initialProperties: {
            ...
            qHyperCubeDef: {
                ...
                qSocket: [{
                    qSocketServiceUrl: "wss://win-8ebv7me9ruc:12345"
                ...
                settings: {
                    ...
                    items: {
                        ...
                        socketService: {  
                            ...
                            type: "string",
                            defaultValue: "wss://win-8ebv7me9ruc:12345"
        ...
        paint: function($element, layout) {
            ...
            var html = "<div id='notifybar' align='center' style='padding: 5px 5px 5px 5px;' class='notifystatus'><div id='testnotify'></div></div><div border='1px'><div class='row fixed-table'><div class='table-content' border='2px'><table class='table table-striped table-fixed-header table-bordered table-responsive table-hover'><thead><tr>";
            var socket = new WebSocket(socketServiceUrl);//"wss://192.168.124.99:12345"); //);
            //ws.connect();
            ...
            socket.onmessage = function(event) {
             notifybaridold = notifybarid;
             //$('#notifybar'+notifybaridold).remove();
             notifybarid += 1;
             var currentstatusbar = $('#testnotify').clone();
             currentstatusbar.attr('id','testnotify'+notifybarid)

             // console.log('notifybarid: '+notifybarid)
             // console.log('notifybaridold: '+notifybaridold)

             if (event.data === 'Data Submitted To Sql') {
             	// $("#notifybar").fadeIn(500);
             	// currentstatusbar.show()
             	// currentstatusbar.fadeIn(500);
             	// currentstatusbar.innerText=event.data;
             	// console.log(event.data)
             	// console.log(currentstatusbar.innerText)
             	// console.log(currentstatusbar)
             	//currentstatusbar.style.background='#FFF68F';
				$("#notifybar").fadeIn(500);
             	$("#notifybar")[0].innerText=event.data;
             	$("#notifybar")[0].style.background='#FFF68F';
             	 
             } else if (event.data === 'Reload Submitted.') {
             	$("#notifybar")[0].innerText=event.data;
             	$("#notifybar")[0].style.background='#FFF68F';
             } else if (event.data === 'Task Reload Started') {
             	$("#notifybar")[0].innerText=event.data;
	             //console.log('if event.data - task reload started');
	             $("#notifybar").append("<img src='/Extensions/wb/ajax-loader.gif' width='15%' id='loadingimg'>").attr('valign','middle')
	             $("#notifybar")[0].style.background='#FFF68F';
	         } else if ( event.data === 'Data Submission Succeeded' ) {
	         	$("#notifybar")[0].innerText=event.data;
	         	 //console.log('if event.data -  data submission succeeded');
	         	 $("#notifybar")[0].style.background='#ccffcc';
	         }  else  if (event.data === 'Task Reload Succeeded') {
	         	$("#loadingimg").remove();
	         	$("#notifybar")[0].innerText=event.data;
	         	$("#notifybar")[0].style.background='#ccffcc';
	         	$("#notifybar").delay(2000).fadeOut(1000);
			 } else	{
			 	$("#notifybar").fadeIn(500);
			 	$("#notifybar")[0].innerText=event.data;
			 	//console.log('lastmsgrcvd else: '+lastmsgrcvd)
	         	$("#notifybar").delay(5000).fadeOut(1000);
	         }

             //console.log('got message2: '+event.data)
            };
            ...
            function postData(webServiceUrl, obj) {
                ...
                $.ajax({
                    ...
                    success: function(msg) {
                         ...
                         //$("#notifybar")[0].delay(5000).fadeOut(400);
